...
<template>
  <v-navigation-drawer elevation="2" expand-on-hover>
      ...
      <v-list-item prepend-avatar="https://randomuser.me/api/portraits/women/44.jpg" title="ABC" subtitle="abc@gmail.com">
        <template #append>
          <div class="justify-self-end">
            <v-btn rounded="xl" :icon="layoutStore.state.isRail ? 'mdi-pin-off-outline' : 'mdi-pin-outline'"
    ...
    <v-divider />
    <v-list v-model:opened="open" color="primary" density="compact" variant="plain" nav :lines="false">
      <v-list-item prepend-icon="mdi-home" title="Home" value="/" to="/" />
      ...
      <v-list-group value="Users">
        ...
        <v-list-item v-for="({ title, to }, i) in cruds" :key="i" :value="to" :to="to" :title="title" />
      </v-list-group>

      <v-list-group value="Users2">
        <template #activator="{ props }">
          <v-list-item v-bind="props" prepend-icon="mdi-test-tube" title="Menu 2" />
        </template>
        <!-- <v-list-subheader>REPORTS</v-list-subheader> -->

        <v-list-item v-for="({ title, to }, i) in cruds2" :key="i" :value="to" :to="to" :title="title" />
      </v-list-group>
    </v-list>
